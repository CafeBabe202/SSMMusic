...
<section id="content" class="m-t-lg wrapper-md animated fadeInDown">
    <div class="container aside-xl">
        <a class="navbar-brand block" href="<%=basePath%>"><span class="h1 font-bold">Guide Music</span></a>
        <section class="m-b-lg">
            ...
            <form action="<%=basePath%>/user/register" method="post">
                <div class="form-group">
                    <input placeholder="用户名 4~16位" class="form-control rounded input-lg text-center no-border"
                           name="userName" id="userName">
                    <span id="cheName"></span>
                </div>
                <div class="form-group">
                    <input type="email" placeholder="邮件" class="form-control rounded input-lg text-center no-border"
                           name="email">
                </div>
                <div class="form-group">
                    <input type="password" placeholder="密码" class="form-control rounded input-lg text-center no-border"
                           name="password" id="userPwd">
                ...
                <div class="checkbox i-checks m-b">
                    <label class="m-l">
                        <input type="checkbox" checked="" value="yes" id="rule"><i></i> 同意 <a href="#">隐私政策</a>
                    </label>
                </div>
                <button type="submit" class="btn btn-lg btn-warning lt b-white b-2x btn-block btn-rounded"
                        onclick="return submitTest()"><i class="icon-arrow-right pull-right"></i><span class="m-r-n-lg">Sign up</span>
                </button>
                <div class="line line-dashed"></div>
                <p class="text-muted text-center"><small>已经有账号?</small></p>
                <a href="<%=basePath%>login.jsp" class="btn btn-lg btn-info btn-block btn-rounded">登录</a>
            </form>
        </section>
    </div>
</section>
...
<script type="text/javascript">
    ...
    $('#userName').blur(function () {
        ...
        $.get("<%=basePath %>/user/checkName", {"userName": userName}, function (data) {
            if (userName === "") {
    ...
    function submitTest() {
        console.log(uname_ok,pwd_ok,confirm_pwd_ok,rule_ok);
