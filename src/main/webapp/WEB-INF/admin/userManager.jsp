                                    ...
                                    <c:if test="${not empty info.list }">
                                        <c:forEach var="m" items="${info.list}">
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="javascript:doRemove(${m.uid},'${m.userName}')"
                                    ...
                                    <tr>
                                        <th colspan="5">
                                            <div class="inline pull-right page">
                                                <a href="<%=basePath%>/user/toUserManager?pageNO=1">第一页</a>

                                                <c:if test="${info.isFirstPage}">
                                                    上一页
                                                </c:if>
                                                <c:if test="${!info.isFirstPage}">
                                                    <a href="<%=basePath%>/user/toUserManager?pageNO=${info.prePage}">上一页</a>
                                                </c:if>
                                                ...
                                                <c:if test="${!info.isLastPage}">
                                                    <a href="<%=basePath%>/user/toUserManager?pageNO=${info.nextPage}">下一页</a>
                                                </c:if>
                                                <a href="<%=basePath%>/user/toUserManager?pageNO=${info.pages}">最后一页</a>
                                                &nbsp;&nbsp;&nbsp;
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    ...
                                    <!-- 表单嵌套表格：规范表单格式 -->
                                    <form action="insertUser.html " id="form" method="post" role="form">
                                            ...
                                            <table class="table" style="font: '黑体';">
                                                ...
                                                <tr>
                                                    ...
                                                    <td><input type="text" id="UserEmail" name="email"
                                                               class="form-control" step="0.01" maxlength="20"
                                                               required="required"/></td>
                                                </tr>

                                                <tr>
                                                    <td><b>客户密码：</b></td>
                                                    <td><input type="text" id="UserPwd" name="password"
                                                               class="form-control" step="0.01" maxlength="10"/></td>
                                                </tr>

                                                <tr>
                                                    <td><b>客户电话：</b></td>
                                                    <td><input type="text" id="UserPhoto" name="phone"
                                                               class="form-control"/></td>
                                                </tr>

                                                <tr>
                                                    <td><b>客户地址：</b></td>
                                                    <td><input type="text" id="UserAddress" name="address"
                                                               class="form-control"/></td>
                                                </tr>
                                                </tbody>
                                            </table>
...
<script type="text/javascript">
    function editInfo(obj, type) {
        if (type == 0) {
            $("#form").attr("action", "<%=basePath%>user/saveUser");
            $("#op").val("add");
            $("#UserName").val("");
            $("#UserEmail").val("");
            $("#UserPwd").val("");
            ...
            $("#form").attr("action", "<%=basePath%>user/editUser");
            $("#op").val("edit");
    ...
    function doRemove(uid, name) {
        if (confirm("您确定删除  用户：" + name + " 吗？")) {
            $.post("<%=basePath%>user/delById", {'userId': uid}, function (data) {
                if (data == "yes") {
                    alert("删除成功");
