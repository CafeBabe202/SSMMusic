                                    ...
                                    <c:if test="${not empty info }">
                                        <c:forEach var="m" items="${info.list }">
                                            <tr>
                                                ...
                                                <td>${m.videoname }</td>
                                                <td>${m.videoauthor }</td>
                                                <td><img src="/musicfile/${m.videophoto }" width="50px"
                                                         height="50px"></td>
                                                <td>${m.videourl }</td>
                                                ...
                                                <td>
                                                    <a href="javascript:doRemove(${m.videoid},'${m.videoname }')"
                                    ...
                                    <tr>
                                        <th colspan="5">
                                            <div class="inline pull-right page">
                                                <a href="<%=basePath%>/video/toVideoManager?pageNO=${info.firstPage}">第一页</a>

                                                <c:if test="${info.isFirstPage}">
                                                    上一页
                                                </c:if>
                                                <c:if test="${!info.isFirstPage}">
                                                    <a href="<%=basePath%>/video/toVideoManager?pageNO=${info.prePage}">上一页</a>
                                                </c:if>
                                                ...
                                                <c:if test="${!info.isLastPage}">
                                                    <a href="<%=basePath%>/video/toVideoManager?pageNO=${info.nextPage}">下一页</a>
                                                </c:if>
                                                <a href="<%=basePath%>/video/toVideoManager?pageNO=${info.lastPage}">最后一页</a>
                                                &nbsp;&nbsp;&nbsp;
                                                共<span class='current'>${info.pages}</span>条记录<span
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    ...
                                    <!-- 表单嵌套表格：规范表单格式 -->
                                    <form action="insertVideo.html" id="form" method="post" role="form"
                                          enctype="multipart/form-data">
                                        <input type="hidden" id="op" name="op">
                                        <input type="text" id="vid" name="videoid">
                                        <div class="form-group">
                                            <table class="table" style="font: '黑体';">
                                                ...
                                                <tr>
                                                    <td><b>视频主角:</b></td>
                                                    <td><input type="text" id="videoAuthor" name="videoauthor"
                                                               class="form-control"/>
                                                    </td>
                                                </tr>
                                                <tr>
...
<script type="text/javascript">
    function editInfo(obj, type) {
        if (type == 0) {
            $("#form").attr("action", "<%=basePath%>video/saveVideo");
            $("#op").val("add");
            $("#videoName").val("");
            $("#videoAuthor").val("");
            $("#videoPhoto").val("");
            ...
            $("#form").attr("action", "<%=basePath%>video/editVideo");
            $("#op").val("edit");
    ...
    function doRemove(videoId, name) {
        if (confirm("您确定删除 " + name + " 吗？")) {
            $.post("<%=basePath%>video/delVideoById", {"videoId": videoId}, function (data) {
                alert("删除成功");
